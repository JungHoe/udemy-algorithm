...
function someRecursive(arr: number[], fn: Function): boolean {
  if (arr.length === 0) {
    return false;
  }
...
type recursiveArrayType = Array<recursiveArrayType | number>;

function flatten(arr: recursiveArrayType): number[] {
  let result: number[] = [];
  function innerFlatten(idx: number) {
    const item = arr[idx];
...
function capitalizeWords(arr: string[]): string[] | string {
  let result: string[] = [];
  if (arr.length === 1) {
    return arr[0].toUpperCase();
  }
  for (let i = 0; i < arr.length; i += 1) {
    const temp: string[] = arr.slice(i, i + 1);
    result.push(capitalizeWords(temp) as string);
  }
